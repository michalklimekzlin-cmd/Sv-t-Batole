// BatoleSvět v0.31 — voice + bráškové + AI Workshop + BroNet + FAB/Toast
...
let state = JSON.parse(localStorage.getItem(SKEY) || 'null') || {
  ...
  companionName:null,
  persona:'orbit',   // 'orbit'|'miza'|'iskra'|'ferum'
...
/* ---------- BroNet: vnitřní síť agentů (předvídání) ---------- */
const BroNet = {
  subs:{},
  publish(topic, payload){ (this.subs[topic]||[]).forEach(fn=>{ try{ fn(payload); }catch(e){} }); },
...
BroNet.subscribe('pathChanged', (path)=>{
  ...
  if(path==='key'){
    enqueueHint('Za chvíli zkus přesnou Otázku – přidá jasnost.');
    enqueuePreAct(()=>{ const i=Math.floor(Math.random()*state.nodes.length); neighbors(i).forEach(n=> state.nodes[n].level+=1); });
  } else if(path==='lock'){
    enqueueHint('Drž rytmus. Když to bude křehké, zpevním aktivní.');
    enqueuePreAct(()=>{ state.nodes.filter(n=>n.active).forEach(n=> n.level+=1); });
  } else if(path==='eagle'){
    enqueueHint('Zkus tkaní s Orlem – roznesu jiskry do okolí.');
    enqueuePreAct(()=>{ const i=Math.floor(Math.random()*state.nodes.length); neighbors(i).forEach(n=> state.nodes[n].level+=1); });
  }
});

/* ---------- PERSONAS (včetně Ferum) ---------- */
let PERSONAS = {
  ...
  ferum:{ label:'🛡️ Ferum', intro:'Držím linii. Tvoř klidně, já pohlídám rám.',      style:{humor:0.25, empathy:0.6, tempo:0.92}, voiceHint:/cs|sk/i }
};
function personaName(){ 
  if(state.companionName) return state.companionName;
  switch(state.persona){ case 'miza': return 'Míza'; case 'iskra': return 'Iskra'; case 'ferum': return 'Ferum'; default: return 'Orbit'; }
}
function personaSay(text){
  chatPush('ai', `${personaName()}: ${text}`);
  if(state.voice.enabled) speak(text);
}
...
function handleChoice(id){
  ...
  switch(id){
    ...
    case 'path_key':
      state.path='key'; personaSay('Zvoleno: Cesta Klíče. Občas ti rozsvítím jasnost.');
      BroNet.publish('pathChanged','key'); break;
    case 'path_lock':
      state.path='lock'; personaSay('Zvoleno: Cesta Zámku. Zpevním aktivní uzly, když to bude křehké.');
      BroNet.publish('pathChanged','lock'); break;
    case 'path_eagle':
      state.path='eagle'; personaSay('Zvoleno: Cesta Orla. Roznesu jiskry do okolí, až to bude dávat smysl.');
      BroNet.publish('pathChanged','eagle'); break;
    case 'ask_mission':
...
function aiRespond(){
  ...
  // --- BroNet předvídání (jemné, bez cheatování) ---
  if(preActQueue.length){
    const act = preActQueue.shift();
    try{ act(); }catch(e){}
    state.nodes.forEach(n=> n.active = n.level>=2);
  }
  if(hintQueue.length){
    const tip = hintQueue.shift();
    personaSay(tip);
  }
...
const WS = {
  ...
  propose(){
    const scenarios = [ ()=>patchAddJokesForIskra(), ()=>patchBoostFerumShield(), ()=>patchEnableShopBtn() ];
...
function patchAddJokesForIskra(){
  return {
    ...
    log:[ 'Plánovač: Iskra může víc vtipkovat.', 'Kodér: Přidány 2 hlášky.', 'Recenzent: Safe, text-only.' ]
  };
}
function patchBoostFerumShield(){
  return {
    kind:'ferum_shield_boost',
    payload:{ bonus:1 },
    log:[ 'Plánovač: Ferum občas zpevní aktivní uzly.', 'Kodér: Přidán +1 při nudge.', 'Recenzent: Safe.' ]
  };
}
function patchEnableShopBtn(){
  return {
    kind:'enable_shop_button',
    payload:{ enable:true },
    log:[ 'Plánovač: Hráč chce obchod.', 'Kodér: Povoleno tlačítko Shop.', 'Recenzent: UI-only.' ]
  };
}
...
function applyLastPatch(){
  ...
  switch(p.kind){
    case 'extend_persona_lines': {
      ...
      PERSONAS[who].extras = PERSONAS[who].extras || {}; PERSONAS[who].extras.jokes = (PERSONAS[who].extras.jokes||[]).concat(jokes);
      wsWrite(`✅ Přidány vtipy pro ${who}.`, 'ok'); applied=true; break;
    }
    case 'ferum_shield_boost': {
      PERSONAS.ferum.extras = PERSONAS.ferum.extras || {}; PERSONAS.ferum.extras.shieldBonus = (PERSONAS.ferum.extras.shieldBonus||0) + (p.payload.bonus||1);
      wsWrite('✅ Ferum posílen (štít).', 'ok'); applied=true; break;
    }
    case 'enable_shop_button': {
      const btn=$('#shopBtn'); if(btn){ btn.disabled=false; btn.title=''; btn.onclick=()=>alert('🛍️ Obchod bude ve v0.4: tuning hlasu, +1 energy cap, kosmetika mapy.'); }
      wsWrite('✅ Obchod povolen (placeholder).', 'ok'); applied=true; break;
    }
  }
...
function undoPatch(){
  ...
  wsWrite('↩️ Vráceno (UI se obnoví).', 'warn'); save(); location.reload();
}
...
function wsWrite(text,cls='ok',replace=false){
  const el=$('#wsLog'); if(replace) el.innerHTML=''; const div=document.createElement('div'); div.className=cls; div.textContent=text; el.appendChild(div); el.scrollTop=el.scrollHeight;
}

/* vtipy po akcích */
function maybeTellJoke(){
  const p = PERSONAS[state.persona]; const jokes = (p && p.extras && p.extras.jokes) ? p.extras.jokes : [];
...
/* helpers */
function renderChat(){ const list=state.chat.log.slice(0,50).map(m=>`<div class="msg ${m.who==='ai'?'ai':'me'}"><div class="bubble">${escapeHtml(m.text)}</div></div>`).join(''); $('#chatLog').innerHTML=list; $('#chatLog').scrollTop=$('#chatLog').scrollHeight; }
function escapeHtml(s){ return String(s).replace(/[&<>"']/g, (m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;','\'':'&#39;'}[m])) ); }
